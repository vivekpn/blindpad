...
var TIME_PATH_SEPARATOR = '#';
var PATH_SEPARATOR = ',';
var DIGIT_REPLICA_SEPARATOR = '*';
/**
...
var Ident = (function () {
    ...
    Ident.parse = function (str) {
        try {
            var _a = str.split(TIME_PATH_SEPARATOR), time = _a[0], pathstr = _a[1];
            ...
            var path = pathstr.split(PATH_SEPARATOR).map(function (token) {
                var _a = token.split(DIGIT_REPLICA_SEPARATOR, 2), digit = _a[0], replica = _a[1];
                if (!replica) {
                    replica = prev_1;
                }
                else {

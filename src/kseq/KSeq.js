...
var KSeq = (function () {
     ...
     */
    function KSeq(name, timestampFn, randomFn, atoms, identGenerator) {
        this.name = name;
        this.time = 0;
        this.atoms = atoms || new storage_1.ArrayAtomList();
        this.removed = new idents_1.IdentSet();
        this.identGenerator = identGenerator || new idents_1.LSEQIdentGenerator(randomFn);
    ...
    KSeq.prototype.insert = function (value, pos) {
        ...
        var op = new Op_1.InsertOp(this.name, this.timestampFn(), id, value);
        this.apply(op);
        return op;
    };
    /**
    ...
    KSeq.prototype.remove = function (pos) {
        ...
        if (atom) {
            var op = new Op_1.RemoveOp(this.name, this.timestampFn(), atom.id);
            this.apply(op);
            return op;
        }
        return null;
